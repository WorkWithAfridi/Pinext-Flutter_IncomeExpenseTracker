...
import 'package:pinext/app/app_data/packages/live_animated_widgets/src/animator/stack.dart';
...
class _AnimateIfVisibleState extends State<AnimateIfVisible> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    final itemAlreadyShowed = _wrapper!.stack!.isAlreadyAnimated(widget.key) && !widget.reAnimateOnVisibility;
  ...
  void _visibilityChanged(VisibilityInfo info) {
    if (info.visibleFraction > widget.visibleFraction && !_controller.isCompleted) {
    ...
    } else if (info.visibleFraction <= widget.visibleFraction && mounted && widget.reAnimateOnVisibility && !info.visibleBounds.isEmpty) {
      _controller.reverse();
    } else if (info.visibleFraction <= widget.visibleFraction && mounted && widget.reAnimateOnVisibility && info.visibleBounds.isEmpty) {
      _controller.reset();
    }
...
class _VisibilityStackProvider extends InheritedWidget {
  ...
  static _VisibilityStackProvider? of(BuildContext context) =>
      context.getElementForInheritedWidgetOfExactType<_VisibilityStackProvider>()?.widget as _VisibilityStackProvider?;

  @override
  bool updateShouldNotify(_VisibilityStackProvider oldWidget) => oldWidget.stack != stack;
}
