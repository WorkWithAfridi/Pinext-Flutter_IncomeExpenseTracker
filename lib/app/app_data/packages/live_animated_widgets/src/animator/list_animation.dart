...
class ActiveItem implements Comparable<ActiveItem> {
  ActiveItem.incoming(this.controller, this.itemIndex) : removedItemBuilder = null;
...
mixin ListAnimation<T extends StatefulWidget> on State<T> {
  ...
  void insertItem(int index, {Duration duration = _kDuration}) {
    ...
    controller.forward().then<void>((_) {
      removeActiveItemAt(incomingItems, incomingItem.itemIndex)!.controller!.dispose();
    });
  }
  ...
  void removeItem(
    ...
    final controller = incomingItem?.controller ?? AnimationController(duration: duration, value: 1, vsync: vsync);
    ...
    controller.reverse().then<void>((void value) {
      removeActiveItemAt(outgoingItems, outgoingItem.itemIndex)!.controller!.dispose();
